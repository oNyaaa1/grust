...
function clearSlot(ply)
    ...
    -- Remove all weapons
    for i = 1, 36 do
        if ply.Inventory[i] and ply.Inventory[i].Weapon then
            if ply:HasWeapon(ply.Inventory[i].Weapon) then
                ply:StripWeapon(ply.Inventory[i].Weapon)
            end
        end
    end
    ...
    for i = 1, 36 do
        ...
        net.WriteTable({Weapon = nil, Mats = nil})
        net.WriteFloat(i)
        net.WriteBool(false)
        net.Send(ply)
    end
...
hook.Add("PlayerDeath", "sAndbox_DeathClearInventory", function(ply)
    clearSlot(ply)
end)