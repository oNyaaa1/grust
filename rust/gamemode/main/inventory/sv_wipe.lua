...
local function SaveProps()
    ...
    for _, ent in ipairs(ents.GetAll()) do
        if IsValid(ent) and ent:GetClass() ~= "player" and ent:GetClass() ~= "worldspawn" then
            ...
            local isSent = string.find(ent:GetClass(), "sent_")
...
local function LoadProps()
    ...
    for _, ent in ipairs(ents.GetAll()) do
        if IsValid(ent) and ent:GetClass() ~= "player" and ent:GetClass() ~= "worldspawn" then
            local hasSocket = ent.GetSocket and ent:GetSocket()
            local isSent = string.find(ent:GetClass(), "sent_")
            if hasSocket or isSent then ent:Remove() end
        end
    end
...
local function SaveAll()
    ...
    for _, ply in ipairs(player.GetAll()) do
        if IsValid(ply) and ply:SaveInventory() then playerCount = playerCount + 1 end
    end
...
hook.Add("PlayerDisconnected", "sAndbox_SaveOnDisconnect", function(ply) if IsValid(ply) then ply:SaveInventory() end end)
...
concommand.Add("sandbox_load", function(ply, cmd, args)
    ...
    for _, pl in ipairs(player.GetAll()) do
        if IsValid(pl) then pl:LoadInventory() end
    end
